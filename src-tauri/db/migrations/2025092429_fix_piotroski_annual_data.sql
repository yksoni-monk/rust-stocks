-- Migration: Fix Piotroski F-Score views to use Annual data instead of TTM
-- Created: 2024-09-24
-- Purpose: Connect EDGAR Annual data with Piotroski calculations for better accuracy

-- Drop existing Piotroski views that use TTM data
...
DROP VIEW IF EXISTS piotroski_f_score_working;
DROP VIEW IF EXISTS piotroski_f_score_simple;

-- Create the multi-year data view using Annual data
...
SELECT
    ...
    -- 8. Improving Gross Margin - requires historical data
    CASE
...
-- Create simplified screening results view
