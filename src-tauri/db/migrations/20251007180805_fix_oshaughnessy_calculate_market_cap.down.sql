-- Revert to previous version that read market_cap from daily_prices
-- This is from migration 2025123107
...
SELECT
  ...
  ((SELECT market_cap FROM daily_prices WHERE stock_id = s.id ORDER BY date DESC LIMIT 1) + COALESCE(b.total_debt, 0) - COALESCE(b.cash_and_equivalents, 0)) as enterprise_value,
  (COALESCE(i.operating_income, 0) + COALESCE(cf.depreciation_expense, 0) + COALESCE(cf.amortization_expense, 0)) as ebitda,
  CASE WHEN i.net_income > 0 THEN (SELECT market_cap FROM daily_prices WHERE stock_id = s.id ORDER BY date DESC LIMIT 1) / i.net_income ELSE NULL END as pe_ratio,
  CASE WHEN b.total_equity > 0 THEN (SELECT market_cap FROM daily_prices WHERE stock_id = s.id ORDER BY date DESC LIMIT 1) / b.total_equity ELSE NULL END as pb_ratio,
  CASE WHEN i.revenue > 0 THEN (SELECT market_cap FROM daily_prices WHERE stock_id = s.id ORDER BY date DESC LIMIT 1) / i.revenue ELSE NULL END as ps_ratio,
  CASE WHEN i.revenue > 0 THEN ((SELECT market_cap FROM daily_prices WHERE stock_id = s.id ORDER BY date DESC LIMIT 1) + COALESCE(b.total_debt, 0) - COALESCE(b.cash_and_equivalents, 0)) / i.revenue ELSE NULL END as evs_ratio,
  CASE WHEN (COALESCE(i.operating_income, 0) + COALESCE(cf.depreciation_expense, 0) + COALESCE(cf.amortization_expense, 0)) > 0 THEN ((SELECT market_cap FROM daily_prices WHERE stock_id = s.id ORDER BY date DESC LIMIT 1) + COALESCE(b.total_debt, 0) - COALESCE(b.cash_and_equivalents, 0)) / (COALESCE(i.operating_income, 0) + COALESCE(cf.depreciation_expense, 0) + COALESCE(cf.amortization_expense, 0)) ELSE NULL END as ev_ebitda_ratio,
  CASE WHEN (SELECT market_cap FROM daily_prices WHERE stock_id = s.id ORDER BY date DESC LIMIT 1) > 0 THEN (COALESCE(cf.dividends_paid, 0) + COALESCE(cf.share_repurchases, 0)) / (SELECT market_cap FROM daily_prices WHERE stock_id = s.id ORDER BY date DESC LIMIT 1) ELSE NULL END as shareholder_yield,
  ((CASE WHEN i.net_income > 0 THEN 1 ELSE 0 END) + (CASE WHEN b.total_equity > 0 THEN 1 ELSE 0 END) + (CASE WHEN i.revenue > 0 THEN 1 ELSE 0 END) + (CASE WHEN i.revenue > 0 THEN 1 ELSE 0 END) + (CASE WHEN (COALESCE(i.operating_income, 0) + COALESCE(cf.depreciation_expense, 0) + COALESCE(cf.amortization_expense, 0)) > 0 THEN 1 ELSE 0 END) + (CASE WHEN (SELECT market_cap FROM daily_prices WHERE stock_id = s.id ORDER BY date DESC LIMIT 1) > 0 THEN 1 ELSE 0 END)) * 16.67 as data_completeness_score
FROM stocks s
