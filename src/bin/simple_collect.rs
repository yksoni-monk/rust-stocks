...
async fn main() -> Result<()> {
    ...
    if args.len() < 2 {
        println!("Usage: cargo run --bin simple_collect -- <start_date> [end_date]");
        println!("Example: cargo run --bin simple_collect -- 20250810");
        return Ok(());
    }
    ...
    println!("🚀 Simple Stock Data Collection");
    println!("📅 Fetching data from {} to {}", start_str, end_str);

    // Parse dates
    let start_date = parse_date(start_str)?;
    let end_date = parse_date(end_str)?;

    println!("✅ Dates parsed: {} to {}", start_date, end_date);
    ...
    for (i, stock) in test_stocks.iter().enumerate() {
        ...
        match schwab_client.get_price_history(&stock.symbol, start_date, end_date).await {
            Ok(bars) => {
