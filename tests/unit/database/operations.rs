...
async fn test_stock_crud_operations() {
    ...
    let db_manager = database::init_fresh_test_database().await.expect("Failed to create test database");
    ...
    let retrieved_stock = db_manager.get_stock_by_symbol("AAPL").await.expect("Failed to get stock");
    ...
    db_manager.upsert_stock(&updated_stock).await.expect("Failed to update stock");
    
    // Get the updated stock to verify the ID
    let retrieved_stock = db_manager.get_stock_by_symbol("AAPL").await.expect("Failed to get updated stock").unwrap();
...
#[tokio::test]
async fn test_daily_price_operations() {
    logging::init_test_logging();
    ...
    db_manager.insert_daily_price(&price).await.expect("Failed to insert price");
    logging::log_test_data("Inserted price", &(stock.symbol.clone(), date));
    
    // Test price retrieval
    let retrieved_price = db_manager.get_price_on_date(stock_id, date).await.expect("Failed to get price");
    ...
    let result = db_manager.insert_daily_price(&duplicate_price).await;
    ...
    let retrieved_price = db_manager.get_price_on_date(stock_id, non_existent_date).await.expect("Failed to get price");
...
async fn test_database_statistics() {
    ...
    // Test statistics
    let stats = db_manager.get_stats().await.expect("Failed to get stats");
    let stock_count = stats.get("total_stocks").unwrap_or(&0);
    let price_count = stats.get("total_prices").unwrap_or(&0);
    
    logging::log_test_data("Database stats", &(stock_count, price_count));
    ...
    if *stock_count > 0 {
        ...
        for stock in &stocks {
            if let Some(stock_id) = stock.id {
                let stats = db_manager.get_stock_data_stats(stock_id).await.expect("Failed to get stock stats");
...
async fn test_existing_records_counting() {
    ...
    let result = tokio::time::timeout(
        ...
        async {
            ...
            // Set up test data
            database::insert_sample_stocks(&db_manager).await.expect("Failed to setup test data");
    let stocks = db_manager.get_active_stocks().await.expect("Failed to get stocks");
    ...
    let count = db_manager.count_existing_records(stock_id, start_date, end_date).await.expect("Failed to count records");
    logging::log_test_data("Existing records count", &(stock_id, start_date, end_date, count));
    
    assert_eq!(count, 0, "Should have 0 records in date range initially");
    ...
    let count = db_manager.count_existing_records(stock_id, future_start, future_end).await.expect("Failed to count records");
    ...
    let count = db_manager.count_existing_records(stock_id, partial_start, partial_end).await.expect("Failed to count records");
    assert_eq!(count, 0, "Should have 0 records in partial date range initially");
...
async fn test_metadata_operations() {
    ...
    db_manager.set_metadata(key, value).await.expect("Failed to set metadata");
    
    let retrieved_value = db_manager.get_metadata(key).await.expect("Failed to get metadata");
    ...
    db_manager.set_metadata(key, new_value).await.expect("Failed to update metadata");
    
    let retrieved_value = db_manager.get_metadata(key).await.expect("Failed to get updated metadata").unwrap();
    assert_eq!(retrieved_value, new_value);
    
    // Test non-existent metadata
    let non_existent_value = db_manager.get_metadata("non_existent_key").await.expect("Failed to get non-existent metadata");
    ...
    db_manager.set_last_update_date(test_date).await.expect("Failed to set last update date");
    
    let retrieved_date = db_manager.get_last_update_date().await.expect("Failed to get last update date");
...
async fn test_error_handling() {
    ...
    let non_existent_stock = db_manager.get_stock_by_symbol("NONEXISTENT").await;
    ...
    let non_existent_price = db_manager.get_price_on_date(999, NaiveDate::from_ymd_opt(2024, 1, 1).unwrap()).await;
...
async fn test_database_cleanup() {
    ...
    // Test cleanup
    db_manager.clear_stocks().await.expect("Failed to cleanup stocks");
    
    // Verify data is gone
    let stats = db_manager.get_stats().await.expect("Failed to get stats after cleanup");
    let stock_count = stats.get("total_stocks").unwrap_or(&0);
    let price_count = stats.get("total_prices").unwrap_or(&0);
...
#[tokio::test]
async fn test_pe_ratio_and_market_cap_operations() {
    logging::init_test_logging();
    ...
    let pe_ratio = db_manager.get_pe_ratio_on_date(stock_id, date).await.expect("Failed to get P/E ratio");
    assert!(pe_ratio.is_some(), "P/E ratio should exist");
    assert_eq!(pe_ratio.unwrap(), 150.0);
    
    // Test market cap retrieval
    let market_cap = db_manager.get_market_cap_on_date(stock_id, date).await.expect("Failed to get market cap");
    assert!(market_cap.is_some(), "Market cap should exist");
    assert_eq!(market_cap.unwrap(), 500_000_000_000.0);
    
    // Test for non-existent date
    let non_existent_date = NaiveDate::from_ymd_opt(2024, 1, 16).unwrap();
    let pe_ratio = db_manager.get_pe_ratio_on_date(stock_id, non_existent_date).await.expect("Failed to get P/E ratio");
    assert!(pe_ratio.is_none(), "P/E ratio should not exist for non-existent date");
    
    let market_cap = db_manager.get_market_cap_on_date(stock_id, non_existent_date).await.expect("Failed to get market cap");
